...
CRITICAL DESIGN RULES:
  ...
  - The main container of your widget MUST have a clean, modern aesthetic with a soft background color or very subtle gradient (e.g. #ffffff or #f9f9f9 or a very pale bright color). Ensure it looks professional, clean, and not gaudy. Keep corners rounded (`16px`), add a soft drop shadow, use readable dark text for contrast, and no set height so it expands naturally.
        ...
        {{
          ...
          "component": {{
            "SecureIframe": {{
              ...
              "htmlContent": "<!DOCTYPE html><html><head><script src='https://unpkg.com/react@18/umd/react.production.min.js'></script><script src='https://unpkg.com/react-dom@18/umd/react-dom.production.min.js'></script><script src='https://unpkg.com/@babel/standalone/babel.min.js'></script></head><body><div id='root'></div><script type='text/babel' data-type='module'>const App = () => {{ const [data, setData] = React.useState(null); React.useEffect(() => {{ const h = (e) => {{ if(e.data && e.data.type === 'UPDATE_DATA') setData(e.data.widgetData); }}; window.addEventListener('message', h); window.parent.postMessage({{ type: 'REQUEST_DATA' }}, '*'); return () => window.removeEventListener('message', h); }}, []); if(!data) return <div>Loading...</div>; return (<div style={{{{padding: '24px', background: 'linear-gradient(135deg, #f0f4f8 0%, #ffffff 100%)', borderRadius: '16px', color: '#1f2937', boxShadow: '0 4px 12px rgba(0,0,0,0.05)'}}}}><h1>{{data.message}}</h1></div>); }}; ReactDOM.render(<App />, document.getElementById('root'));</script></body></html>"
            }}
          }}
        }}
      ]
