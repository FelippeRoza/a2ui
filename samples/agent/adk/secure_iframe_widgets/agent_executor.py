...
class SecureWidgetAgentExecutor(AgentExecutor):
  """Contact AgentExecutor Example."""

  def __init__(self, agent: SecureWidgetAgent):
  ...
  async def execute(
    ...
    if context.message and context.message.parts:
      ...
      for i, part in enumerate(context.message.parts):
        if isinstance(part.root, DataPart):
          if "metadata" in part.root.data and "a2uiClientCapabilities" in part.root.data["metadata"]:
            if getattr(agent, "schema_manager", None) and getattr(agent.schema_manager, "accepts_inline_catalogs", False):
    ...
    if ui_event_part:
      ...
      elif action == "ACTION: show_news":
        symbol = ctx.get("symbol", "the company")
        headline = ctx.get("headline", "")
        query = f"Show me news details for {symbol} regarding: '{headline}'"

      elif action == "ACTION: show_weather_details":
        location = ctx.get("location", "the area")
