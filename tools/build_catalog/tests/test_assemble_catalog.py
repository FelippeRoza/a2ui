...
class TestAssembleCatalog(unittest.TestCase):
    ...
    def test_local_assembly(self):
        ...
        assembler = CatalogAssembler(
            version="0.9", local_basic_catalog_path=str(self.basic_catalog_path)
        )
        result = assembler.assemble(
            "TestCatalog", [str(self.component1_path), str(self.component2_path)]
        )
        ...
        self.assertEqual(
            result["description"],
            f"TestCatalog A2UI catalog, including {self.component1_path.stem},"
            f" {self.component2_path.stem}.",
        )
    ...
    def test_custom_catalog_id(self):
        ...
        result = assembler.assemble(
            "TestCatalog", [str(self.component1_path)], catalog_id=custom_id
        )
    ...
    def test_remote_basic_catalog_fallback(self, mock_urlopen):
        ...
        mock_response.read.return_value = json.dumps({
            "components": {"Text": {"type": "string"}, "RemoteText": {"type": "string"}}
        }).encode("utf-8")
    ...
    def test_extend_basic_catalog(self):
        ...
        result = assembler.assemble(
            "ExtendedCatalog", [str(self.component1_path)], extend_basic=True
        )
    ...
    def test_http_input_assembly(self, mock_urlopen):
        ...
        result = assembler.assemble(
            "CloudCatalog", ["https://example.com/widget_catalog.json"]
        )
        ...
        self.assertEqual(
            ...
            "https://example.com/widget_catalog.json",
        )
    ...
    def test_remote_common_types_fallback(self, mock_urlopen):
        ...
        mock_response = MagicMock()
        mock_response.read.return_value = json.dumps(
            {"$defs": {"DynamicString": {"type": "string"}}}
        ).encode("utf-8")
        mock_urlopen.return_value.__enter__.return_value = mock_response
    ...
    def test_collision_warning_on_merge(self, mock_logger, mock_fetch_json):
        ...
        mock_schema = {"components": {"CustomHeader": {"type": "object"}}}
        mock_fetch_json.return_value = mock_schema

        assembler.assemble(
            "CollisionCatalog", ["memory://file1.json", "memory://file2.json"]
        )

        # We should see a warning about 'CustomHeader' colliding
        mock_logger.warning.assert_called_with(
            "Component collision: 'CustomHeader' already exists. Overwriting."
        )
    ...
    def test_network_timeout(self, mock_urlopen):
        ...
        with self.assertRaises(CatalogError) as context:
            assembler.fetch_json(
                "https://example.com/slow_catalog.json",
                referrer="memory://component.json",
            )
    ...
    def test_output_filename_generation(self, mock_assemble, mock_cwd):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            self.assertFalse(
                (Path(temp_dir) / "dist" / "MyCatalogWithExt.json.json").exists()
            )
    ...
    def test_catalog_json_resolution(self, mock_fetch_json):
        ...
        mock_schema = {
            "components": {
                "TestComp": {
                    "properties": {"func": {"$ref": "catalog.json#/$defs/anyFunction"}}
                }
            }
        }
        mock_fetch_json.return_value = mock_schema
    ...
    def test_synthesized_defs(self):
        ...
        result = assembler.assemble(
            "SynthesizedCatalog", [str(self.component1_path), str(self.component2_path)]
        )
        ...
        self.assertEqual(
            result["$defs"]["anyComponent"]["discriminator"]["propertyName"],
            "component",
        )
        any_comp_refs = [
            item["$ref"] for item in result["$defs"]["anyComponent"]["oneOf"]
        ]
    ...
    def test_theme_property_override_and_clash(self, mock_fetch_json):
        ...
        custom_schema_2 = {
            ...
            "$defs": {
                "theme": {
                    "type": "object",
                    "properties": {"customProp": {"type": "boolean"}},
                }
            },
        }
        ...
        assembler = CatalogAssembler(
            version="0.9", local_basic_catalog_path="memory://basic_catalog.json"
        )
        ...
        with self.assertRaises(CatalogError) as context:
            assembler.assemble(
                "TestThemeClash", ["memory://custom1.json", "memory://custom2.json"]
            )
    ...
    def test_inline_instructions_and_additional_files(self):
        ...
        with patch.object(CatalogAssembler, "fetch_json") as mock_fetch:
            ...
            with patch("pathlib.Path.exists", return_value=True), patch(
                "pathlib.Path.read_text", return_value="External CLI rules."
            ):
                result = assembler.assemble(
                    "TestInstructions",
                    ["memory://custom.json"],
