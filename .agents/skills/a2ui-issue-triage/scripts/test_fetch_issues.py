...
class TestFetchIssues(unittest.TestCase):
    ...
    def test_main_fetch_flow(self, mock_run, mock_parse_args, mock_which, mock_makedirs):
        ...
        def run_cmd_side_effect(args):
            if len(args) >= 3 and args[1] == "api" and "assignees" in args[2]:
                ...
                return "George Spencer <gspencer@example.com>\nVarun S <varun-s10@example.com>"
            ...
            elif len(args) >= 3 and args[1] == "issue" and args[2] == "view" and "comments" in args[-1]:
                return json.dumps({
                    "comments": [
                        {
                            "author": {"login": "Varun-S10"},
    ...
    @patch('argparse.ArgumentParser.parse_args')
    @patch('fetch_issues.run_cmd')
    def test_main_no_issues_exit(self, mock_run, mock_parse_args, mock_which, mock_makedirs):
        mock_which.return_value = "/usr/local/bin/gh"
