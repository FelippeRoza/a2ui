...
- Updated `ComponentsList` in `agent_to_renderer.json` to compose `ComponentCommon` (`$ref: "common_types.json#/$defs/ComponentCommon"`) so base component properties are validated at the envelope level regardless of catalog structure.
